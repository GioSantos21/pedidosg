            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="p-6">
                    ...
                    <div class="mb-6 pb-4 border-b border-gray-200">
                        ...
                        <p class="text-sm text-gray-500 mt-1">Fecha de Solicitud: {{ $order->requested_at->format('d/m/Y H:i') }}</p>
                    </div>

                    <!-- Alpine Data: Inicializa la lógica del formulario dinámico -->
                    <form method="POST" action="{{ route('orders.update', $order) }}"
                          x-data="{
                                // Carga los ítems existentes en la variable de Alpine
                                items: JSON.parse('{{ $currentItems }}'),
                        ...
                        <!-- ITEMS DEL PEDIDO -->
                        <div class="space-y-4">
                            <h3 class="text-xl font-semibold mb-4 border-b pb-2">Productos a Modificar</h3>
                            ...
                            <template x-for="(item, index) in items" :key="index">
                                <div class="flex items-start space-x-4 border border-gray-200 p-4 rounded-lg bg-gray-50">
                                    ...
                                    <div class="flex-1">
                                        <x-input-label :for="'product_' + index" :value="__('Producto')" />
                                    ...
                                    <!-- Cantidad -->
                                    <div class="w-28">
                                        <x-input-label :for="'quantity_' + index" :value="__('Cantidad')" />
                                        <x-text-input :id="'quantity_' + index"
                        ...
                        <div class="mt-8">
                            ...
                            <textarea id="notes" name="notes" rows="3" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full">{{ old('notes', $order->notes) }}</textarea>
