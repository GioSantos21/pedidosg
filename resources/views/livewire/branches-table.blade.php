<div>
    ...
    <div class="p-6 text-gray-900 space-y-4">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-center space-y-4 sm:space-y-0">
            ...
            <a href="{{ route('admin.branches.create') }}" class="bg-[#874ab3] hover:bg-[#623579] text-white font-bold py-2 px-4 rounded">
                Nueva Sucursal
            </a>
            ...
            <div class="w-full sm:w-1/3">
                <input wire:model.live.debounce.300ms="search"
                       type="text"
                       placeholder="Buscar por sucursales, dirección o teléfonos..."
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-4 text-center text-gray-900">
                                    No se encontraron sucursales que coincidan con la búsqueda "{{ $search }}".
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
        ...
        <div class="flex flex-col sm:flex-row justify-between items-center pt-4">
            ...
            <div class="text-sm text-gray-700 mb-4 sm:mb-0">
                @if ($branches->total() > 0)
                    {{ trans('pagination.showing') }} {{ $branches->firstItem() }}
                    {{ trans('pagination.to') }} {{ $branches->lastItem() }}
                    {{ trans('pagination.of') }} {{ $branches->total() }}
